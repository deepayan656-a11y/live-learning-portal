...
export default function StudentDashboard() {
  ...
  const fetchDashboardData = async (token) => {
    ...
    try {
      const [scheduleRes, assignmentRes] = await Promise.all([
        ...
        axios.get(`${API_BASE}/api/v1/assignments`, config)
      ]);

      setUpcomingClasses(Array.isArray(scheduleRes.data) ? scheduleRes.data : []);
  ...
  const handleOpenPeerGallery = async (assignment) => {
    ...
    try {
      const res = await axios.get(`${API_BASE}/api/v1/assignments/${assignment.assignment_id}/submissions`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleAssignmentSubmit = async (e) => {
    ...
    try {
      await axios.post(
        `${API_BASE}/api/v1/assignments/${activeAssignment.assignment_id}/submit`,
        {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-sky-50 to-indigo-50 flex flex-col text-gray-900">
      ...
      <nav className="bg-white/90 backdrop-blur-md shadow-sm border-b border-blue-100 px-6 py-4 flex justify-between items-center sticky top-0 z-40">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-extrabold text-blue-950">Student Portal</h1>
            <p className="text-xs text-blue-600 font-medium">Welcome back, {userName}!</p>
          </div>
        </div>
          ...
          {upcomingClasses.length === 0 ? (
            ...
            <div className="space-y-4">
              {upcomingClasses.map((session) => {
                ...
                return (
                  <div key={session.session_id} className="border border-blue-100 rounded-xl p-4 flex items-center justify-between bg-blue-50/20">
                    <div>
          ...
          {assignments.length === 0 ? (
            ...
            <div className="space-y-4">
              {assignments.map((assignment) => (
                <div key={assignment.assignment_id} className="border border-blue-100 rounded-xl p-4 space-y-3 bg-blue-50/20">
                  ...
                  <div className="flex items-center justify-end gap-3 text-xs pt-2 border-t border-blue-100">
                    <button
      ...
      {activeAssignment && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6 relative border border-blue-100">
            <button 
            ...
            <h3 className="text-lg font-bold text-blue-950">Assignment Workspace</h3>
            <p className="text-xs text-gray-500 mb-4">Submitting for: {activeAssignment.title}</p>

            {submitSuccess && (
              <div className="mb-4 text-xs p-3 rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-200">
                {submitSuccess}
              </div>
            )}
            {submitError && (
            ...
            <form onSubmit={handleAssignmentSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="mt-1 w-full px-3.5 py-2 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500"
                  value={externalLink}
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button
                  ...
                  className="px-4 py-2 border border-gray-200 text-gray-700 rounded-xl text-xs font-medium"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold flex items-center gap-1 shadow-sm"
                >
      ...
      {showGalleryModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full p-6 relative border border-blue-100">
            <button 
            ...
            {publicPeerSubmissions.length === 0 ? (
              ...
              <div className="space-y-3 max-h-80 overflow-y-auto pr-1">
                {publicPeerSubmissions.map((sub, idx) => (
                  <div key={idx} className="bg-blue-50/40 p-4 rounded-xl border border-blue-100">
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-xs font-bold text-blue-900">Featured Student Project</span>
                      {sub.external_link && (
                        <a 
                    ...
                    {sub.instructor_feedback && (
                      <p className="text-xs text-emerald-800 font-medium mt-2 bg-emerald-50 p-2 rounded-lg border border-emerald-100">
